...
const Index = {
    ...
    InXPageTag() {
        try {
            let tag = UiSelector().className('android.widget.FrameLayout').filter(v => {
                return v.isSelected() && v.children().findOne(UiSelector().descContains('按钮'));
            }).isVisibleToUser(true).findOne();

            let child = tag.children().findOne(UiSelector().descContains('按钮'));
    ...
    intoMyPage() {
        ...
        tag = UiSelector().descContains('我').filter(v => {
            return v.bounds().top > Device.height() * 0.8 && v.bounds().left > Device.width() * 0.5 && v.desc().indexOf('我') == 0 && v.parent().isVisibleToUser();
        }).findOne();
        let res = Common.click(tag);//注意，首次打开抖音，这个clickable是false
