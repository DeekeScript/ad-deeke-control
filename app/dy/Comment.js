...
const Comment = {
    ...
    intoShareVideoComment() {
        let imageFile = Images.capture();
        let arr = Images.findTextPosition(imageFile, "评论了");
        if (arr.length == 0) {
            return false;
        }

        Gesture.click(arr[0].left, arr[0].top);
        Common.sleep(1000 + 1000 * Math.random());

        let tag = UiSelector(false).textContains('查看评论').isVisibleToUser(true).findOne();
        if (!tag) {
            return false;
        }
        Common.click(tag);
    ...
    zanShareVideoComment(type) {
        let tag = UiSelector().descContains('赞').descContains('选中').isVisibleToUser(true).findOne();
        if (!tag) {
            return false;
        }

        if (type == 0 && tag.desc().indexOf('已选中') != -1) {
            Log.log('已经点赞了');
            return true;
        }

        if (type == 1 && tag.desc().indexOf('未选中') != -1) {
            Log.log('已经取消点赞了');
            return true;
        }
    ...
    caiShareVideoComment(type) {
        let tag = UiSelector().descContains('踩').descContains('选中').isVisibleToUser(true).findOne();
        if (!tag) {
            return false;
        }
    ...
    reportShareVideoComment() {
        let tag = UiSelector().className('android.widget.FrameLayout').filter(v => {
            return v.desc();
        }).isVisibleToUser(true).findOne();

        Gesture.press(tag.bounds().centerX(), tag.bounds().centerY(), 800);
        System.sleep(3000);

        tag = UiSelector().textContains('举报').isVisibleToUser(true).findOne();
        Common.click(tag);
        System.sleep(3000);

        tag = UiSelector().textContains('不').isVisibleToUser(true).findOne();
        Common.click(tag);
        System.sleep(3000);

        tag = UiSelector().textContains('提交').isVisibleToUser(true).findOne();
