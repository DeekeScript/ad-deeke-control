...
const User = {
    ...
    report(content) {
        let selectTag = UiSelector().textMatches('请选择违规内容').isVisibleToUser(true).findOne();
        Gesture.click(selectTag.bounds().centerX(), selectTag.bounds().centerY());
        System.sleep(3000);
        //选择视频和图集类型
        selectTag = UiSelector().textMatches('视频/图集').isVisibleToUser(true).findOne();
        Gesture.click(selectTag.bounds().centerX(), selectTag.bounds().centerY());
        System.sleep(3000);

        //选择视频和图集内容入口
        let submitTag = UiSelector().textMatches('确认').isVisibleToUser(true).findOne();
        Gesture.click(submitTag.bounds().centerX(), submitTag.bounds().centerY());
        System.sleep(3000);

        selectTag = UiSelector().textMatches('选择视频和图集').isVisibleToUser(true).findOne();
        Gesture.click(selectTag.bounds().centerX(), selectTag.bounds().centerY());
        System.sleep(6000);

        //开始选择前3个视频
        let tags = UiSelector().className('android.widget.CheckBox').find();
        for (let i = 0; i < 3; i++) {
            if (tags[i]) {
                Gesture.click(tags[i].bounds().centerX(), tags[i].bounds().centerY());
                System.sleep(1500);
            }
        }

        submitTag = UiSelector().textMatches('确认').isVisibleToUser(true).findOne();
        Gesture.click(submitTag.bounds().centerX(), submitTag.bounds().centerY());
        System.sleep(3000);

        //填写举报内容
        let iptTag = UiSelector().className('android.widget.EditText').editable(true).findOne();
        iptTag.setText(content);
        System.sleep(2000);
        submitTag = UiSelector().className('android.widget.Button').textMatches('提交').isVisibleToUser(true).findOne();
        Gesture.click(submitTag.bounds().centerX(), submitTag.bounds().centerY());
        System.sleep(3000);

        submitTag = UiSelector().className('android.widget.Button').textMatches('完成').isVisibleToUser(true).findOne();
        if (submitTag) {
            Gesture.click(submitTag.bounds().centerX(), submitTag.bounds().centerY());
            System.sleep(3000);
        }
    ...
    reportUser(first, second, content, taskData) {
        if (this.isBlackUser()) {
            return false;
        }
        ...
        let sendTag = UiSelector().text('举报').isVisibleToUser(true).findOne();
        if (!sendTag) {
            throw new Error('找不到举报按钮');
        }

        let res = sendTag.parent().click();
        Common.sleep(8000 + 2000 * Math.random());
        Log.log("进入举报页面");
        for (let cateName of [first, second]) {
            let tag = UiSelector().filter(v => {
                return v.text() && v.text().indexOf(cateName.substring(0, 4)) !== -1;
            }).findOne();//我不喜欢这个账号及内容
            if (!tag) {
                System.sleep(1500);
                tag = UiSelector().filter(v => {
                    return v.text() && v.text().indexOf(cateName.substring(0, 4)) !== -1;
                }).findOne();//我不喜欢这个账号及内容
            }
            Log.log(tag);
            if (!tag) {
                throw new Error('找不到' + cateName);
            }
            Common.click(tag);
            Common.sleep(3000);
            let submitTag = UiSelector().className('android.widget.Button').filter(v => {
                return v.text() && v.text().indexOf('下一步') !== -1;
            }).findOne();
            if (!submitTag) {
                System.sleep(1500);
                submitTag = UiSelector().className('android.widget.Button').filter(v => {
                    return v.text() && v.text().indexOf('下一步') !== -1;
                }).findOne();
            }
        ...
        let tags = UiSelector().className('android.view.View').filter(v => {
            return v.bounds().width() == v.bounds().height() && v.children().findOne(UiSelector().className('android.widget.TextView').filter(v => {
                return v.text() && v.text().indexOf('/') > 0;
            }));
        }).find();

        for (let i = 0; i < tags.length; i++) {
            Common.selectPhoto(taskData, i, tags[i]);
            let left = 200 + 300 * Math.random();
            let top = 300 + 100 * Math.random();
            Gesture.swipe(left, top + 310, left, top, 200 + 100 * Math.random());
            System.sleep(1500);
            tags = UiSelector().className('android.view.View').filter(v => {
                return v.bounds().width() == v.bounds().height() && v.children().findOne(UiSelector().className('android.widget.TextView').filter(v => {
                    return v.text() && v.text().indexOf('/') > 0;
                }));
            }).find();
