...
const Video = {
    ...
    */
    ...
    report(second, content) {
        let submitTag = UiSelector().textMatches(second).isVisibleToUser(true).filter(v => {
    ...
    reportVideo(first, second, content, taskData) {
        ...
        let index = 0;
        let tags = UiSelector().className('android.view.View').isVisibleToUser(true).filter(v => {
        ...
        Common.selectPhoto(taskData, index, tags[index]);
        ...
        //开始选择内容
        return this.report(second, content);
    },

    reportComment(categoryLabel, taskData) {
        let tag = UiSelector().className('android.widget.FrameLayout').isVisibleToUser(true).descContains('按钮').findOne();
        Gesture.longClick(tag.bounds().left + tag.bounds().width() * Math.random(), tag.bounds().top + tag.bounds().height() * Math.random());
        Common.sleep(3000);
        Log.log('长按评论内容');
        ...
        sendTag.parent().parent().click();
        Log.log('点击举报按钮');

        Common.sleep(4000 + 1000 * Math.random());

        let sTag = UiSelector().text(categoryLabel).isVisibleToUser(true).findOne();
        Gesture.click(sTag.bounds().centerX(), sTag.bounds().centerY());
        Log.log('点击举报类别');
        Common.sleep(3000);

        let submitTag = UiSelector().text('提交').isVisibleToUser(true).findOne();
        let result = Gesture.click(submitTag.bounds().centerX(), submitTag.bounds().centerY());
        Common.sleep(5000);
        Log.log('点击提交按钮', result);
        return result;
    }
}
