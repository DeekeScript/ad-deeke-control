const Common = {
    ...
    backHomeOnly() {
        let i = 0;
        while (i++ < 5) {
            let homeTag = UiSelector().descContains('首页').filter(v => {
                return v.parent().isVisibleToUser();
            }).findOne();
            if (!homeTag) {
                Log.log("没有找到homeTag，返回一次");
                this.back();
                this.sleep(1000);
                continue;
            }
    ...
    */
    ...
    selectPhoto(data, index, tag) {
        //console.log(data.image_groups[0]['images']);
        if (data.image_groups.length == 0) {
            return true;
        }

        if (data.image_groups[index]['images'].length == 0) {
            return true;
        }

        let files = [];
        for (let i = data.image_groups[index]['images'].length - 1; i >= 0; i--) {
            let file = Files.getCachePath() + '/1.png';
            let res = Http.download(data.image_groups[index]['images'][i], file, {});
            let file2 = MediaStore.saveImage(file);//文件移动到相册

            console.log(res, file, file2);
            files.join(file2);
        }
        ...
        let tags = UiSelector().className('android.widget.TextView').isVisibleToUser(true).filter(v => {
            return v.parent().className() == 'android.widget.FrameLayout' && v.parent().bounds().width() == v.parent().bounds().height();
        }).find();

        console.log('开始选择图片', tags.length);
        for (let i = 0; i < data.image_groups[index]['images'].length; i++) {
            Gesture.click(tags[i].bounds().centerX(), tags[i].bounds().centerY());
            System.sleep(500);
        }
        console.log(files);
        System.sleep(1000);
        let sureTag = UiSelector().className('android.widget.TextView').textContains('确认').isVisibleToUser(true).findOne();
        if (!sureTag) {
            Gesture.back();
            System.sleep(1000);
            return false;
        }
        Gesture.click(sureTag.bounds().centerX(), sureTag.bounds().centerY());

        for (let file of files) {
            let success = MediaStore.deleteImage(file);
            if (success) {
                console.log('删除成功');
            }
        }
    ...
    clearGallery() {
        ...
        for (let image of images) {
            console.log(image);
            console.log('删除图片是否成功', MediaStore.deleteImage(image.uri));
        }
        return true;
    },
}
